...
def create_order(body: OrderCreate, user=Depends(get_current_user)):
    ...
    if not cust.data: raise HTTPException(status_code=403, detail="Customer not found")
    payload = body.model_dump()
    if payload.get("delivery_date"): payload["delivery_date"] = str(payload["delivery_date"])
    result = client.table("Orders").insert(payload).execute()
    if not result.data: raise HTTPException(status_code=400, detail="Failed to create order")
